...
const AdminLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try { const r = await axios.post('/api/auth/login',{username,password}); loginAdmin(r.data.token); }
  ...
  return (
    <div style={{minHeight:'100vh',background:'#f8fafc',display:'flex',flexDirection:'column',fontFamily:"'Plus Jakarta Sans',sans-serif"}}>
      <header style={{borderBottom:'1px solid #e2e8f0',padding:'14px 32px',background:'#fff',display:'flex',alignItems:'center',gap:'10px'}}>
        <div style={{width:'28px',height:'28px',background:'#2563eb',borderRadius:'8px',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:700,color:'#fff',fontSize:'13px'}}>{displayName.charAt(0)}</div>
        <span style={{fontWeight:600,fontSize:'15px',color:'#1e293b'}}>{displayName}</span>
      </header>
      <main style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',padding:'40px 24px'}}>
        <div style={{width:'100%',maxWidth:'340px'}}>
          <h1 style={{fontSize:'22px',fontWeight:700,marginBottom:'4px'}}>Admin Login</h1>
          <p style={{color:'#64748b',fontSize:'13px',marginBottom:'28px'}}>{displayName}</p>
          ...
          <div style={{marginTop:'24px',padding:'14px',background:'#fff',border:'1px solid #e2e8f0',borderRadius:'8px'}}>
            <p style={{color:'#64748b',fontSize:'12px',marginBottom:'2px'}}>Are you a student?</p>
            <a href="/student-login" style={{color:'#2563eb',fontSize:'13px',textDecoration:'none',fontWeight:500}}>Student Login →</a>
          </div>
        </div>
      </main>
